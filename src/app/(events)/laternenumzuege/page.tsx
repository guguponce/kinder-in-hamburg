...
import laterneImg from "./laterne.svg";
        ...
        <section className="p-4 rounded-lg bg-gradient-to-b from-hh-950 to-hh-800 w-full flex gap-4 flex-col items-center sm:max-w-[800px] text-white shadow-xl bg-opacity-10 transition-all">
          ...
          <section className="self-start max-w-full">
            <BezirkableEventsList
              ...
              eventsList={laternenEvents}
            />
          </section>
          <article className="md:p-4">
            <DynamicEventsMap
              darkBackground
              thisWeek={laternenEvents.filter(
                (event) =>
                  event.date > today - 1000 * 60 * 60 * 24 &&
                  event.date < nextMonday
              )}
              future={laternenEvents.filter((event) => event.date > nextMonday)}
              today={today}
            />
          </article>
        </section>
