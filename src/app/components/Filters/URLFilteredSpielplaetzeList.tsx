...
export default function URLFilteredSpielplaetzeList({
  children,
  spielplaetzeList,
}: {
  ...
  const [maxDisplayable, setMaxDisplayable] = useState(spielplaetzeList.length);
  const { current: spielplaetzeListRef } = useRef([...spielplaetzeList]);
  ...
  }, [filteredByType, maxDisplay]);

  useEffect(() => {
    setMaxDisplayable(filteredByType.length);
  }, [filteredByType]);
    ...
    <div className="flex gap-2 flex-wrap justify-center items-start w-full">
      <section className="flex flex-col gap-2 flex-grow bg-hh-100 rounded overflow-hidden relative min-h-[100vh] py-2">
        <PostsFilter
          ...
          params={params}
          type={
            type === "type" || type === "bezirke"
              ? (p as "type" | "bezirke")
              : undefined
          }
          queryFromType={p}
          availableCategories={availableTypes}
          categoriesFilter={typeFilter}
          setCategoriesFilter={setTypeFilter}
                    ...
                    <React.Fragment key={bz}>
                      <FilterButton
                        ...
                        typeOnClick={() => {
                          removeFilter({
                            state: typeFilter,
                            arrayStateSetter: setTypeFilter,
            ...
            >
              <div className="w-full h-full rounded-md shadow-2xl overflow-hidden">
                <DynamicGeneralMap>
                  <MarkersLists
                    cluster={false}
                    lists={{ spielplaetze: filteredByType }}
                  />
                </DynamicGeneralMap>
              </div>
            </div>
