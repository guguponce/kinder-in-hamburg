...
export default function UserInputBox({
  ...
  return (
    <section className="userInputBox w-full max-w-[800px] mx-auto justify-center flex flex-wrap items-center gap-2">
      {keys.map((k) => (
        <div className="w-[calc(50%-1rem)] max-w-[300px]" key={k}>
          ...
          >
            {userInput[k] ? (
              ...
              <h3 className="w-full rounded border border-gray-300  px-3 py-1 text-base font-semibold leading-8 text-gray-900 outline-none transition-colors duration-200 ease-in-out focus:border-hh-600 focus:bg-white focus:ring-2 focus:ring-hh-700">
                {userInput[k]}
              </h3>
            )}
          </PostFormInput>
